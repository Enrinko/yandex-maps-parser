...
### Обход капчи через Crawlbase Crawling API (рекомендуется)

Гонять полноценный headful-браузер через прокси Яндекс.Карт тяжело (сотни запросов на страницу,
таймауты). Надёжный путь — [Crawlbase Crawling API](https://crawlbase.com/docs/crawling-api/):
Crawlbase сам рендерит JS, прокручивает страницу и снимает анти-бот на своей стороне и возвращает
готовый HTML, который парсер разбирает теми же селекторами. В корневом `.env` укажите ваш
**JavaScript-токен** Crawlbase (в репозиторий не коммитится):

```
CRAWLBASE_TOKEN=ВАШ_JS_ТОКЕН
```

затем `docker compose up -d` и повторите парсинг. Когда `CRAWLBASE_TOKEN` задан, scraper идёт через
API, а не через локальный браузер; без токена остаётся headful-Chromium (для запусков без капчи).

Альтернатива — обычный residential-прокси, через который ходит сам браузер: `SCRAPER_PROXY`
(парсер при этом включает `ignoreHTTPSErrors`). Подходит для прокси, прозрачно туннелирующих
трафик, но для JS-тяжёлых страниц Crawlbase API надёжнее.
